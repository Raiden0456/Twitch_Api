...
class User {
  async getAllData(Nickname: string) {
    ...
    const totalVideosPagi = await apiClient.helix.videos.getVideosByUserPaginated(userData?.id);
    console.log("Total videos paginated: ", totalVideosPagi);

    // Calculate the user's total videos 
    const totalVideos = await totalVideosPagi.getAll();
    ...
    const videoAmountYear = totalVideos.filter((video) => {
